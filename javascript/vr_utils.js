...
function VisualRecognitionUtils(watson,callback) {
    ...
    if (watson.config.visual_recognition_poker) {
        this.vrServicePoker = wdc.visual_recognition({
            api_key: watson.config.visual_recognition_poker.api_key,
            version: 'v3',
...
VisualRecognitionUtils.prototype.poker_face = function(filePath){
  var deferred = Q.defer();
  var params = {
      images_file: fs.createReadStream(filePath),
      classifier_ids: this.watson.config.visual_recognition_poker.classifier_ids
  };
  this.vrService.classify(params, function(err, res) {
      ...
      {
          console.log("Classification on poker................................");
...
VisualRecognitionUtils.prototype.classify = function(filePath) {
    ...
    this.vrServicePoker.classify(params, function(err, res) {
        if (err)
            deferred.reject(err);
        else
            deferred.resolve(res);
